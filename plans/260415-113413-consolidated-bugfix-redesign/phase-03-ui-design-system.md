...
- **[RED-TEAM] Incremental batch approach** (NOT big-bang grep-replace):
  1. **Batch 1 — Structural colors:** app bg, surface, card bg, borders (commit + visual QA dark+light before Batch 2)
  2. **Batch 2 — Text colors:** primary, secondary, muted (commit + visual QA)
  3. **Batch 3 — Faction colors:** villager/wolf/third glow variants (commit + visual QA)
  4. **Batch 4 — Phase/semantic colors:** night/day/judgment, success/warning/danger (commit + visual QA)
...
- **[RED-TEAM] Per-batch QA checklist** (dark-only after validation):
...
### Design System
- [ ] Implement Moonlit Gothic CSS custom properties (dark mode)
- [ ] Implement light mode `:root:not(.dark)` override
- [ ] Download + self-host Bebas Neue + Inter woff2 fonts
- [ ] Add @font-face declarations with font-display: swap + unicode-range
- [ ] Update Tailwind config: fontFamily + custom color tokens
- [ ] Remove Bungee/Roboto references, update all font-* classes
- [ ] Add global focus-visible styles
- [ ] Add prefers-reduced-motion global reset
- [ ] Remove user-scalable=no from viewport meta

### Components
- [ ] Redesign player card: Glowing Edge borders + faction glow on hover
- [ ] Replace h-44 with min-h-[170px] (responsive per breakpoint)
- [ ] Implement dead player skull overlay + strikethrough + strong dim
- [ ] Redesign action chips: pill-shaped + dot indicator + aria-labels
- [ ] Update card flip to spring physics bezier
- [ ] Merge 2 FABs into 1 expandable FAB (all screen sizes)
- [ ] Redesign timer bar to compact single row + mute button
- [ ] Add drag handle + spring/exit animations to bottom sheet
- [ ] Fix bottom sheet ARIA: role=dialog, aria-modal, aria-hidden backdrop
- [ ] Update faction-theme.ts with Moonlit Gothic colors
- [ ] Fix dead player indicator in SkillSheet target selection (U2)

### Migration
- [ ] Migrate all component color classes to CSS variable references
- [ ] Update responsive breakpoints + card sizing

### Validation
- [ ] Run `npm test` + `npm run build`
- [ ] Visual test: dark mode across all screens
- [ ] Visual test: light mode across all screens
- [ ] Verify WCAG AA contrast (see Section 9.3 table in UI/UX report)
- [ ] Verify fonts render correctly (Vietnamese characters)
- [ ] Verify self-hosted fonts load offline (disconnect network, reload)
- [ ] Test FAB expandable on mobile + tablet + desktop

---

## Success Criteria

- Moonlit Gothic palette active in both dark + light modes
