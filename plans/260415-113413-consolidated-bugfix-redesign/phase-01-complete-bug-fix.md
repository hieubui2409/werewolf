...
### PRE-A1: Storage version marker + executionId migration

<!-- Updated: Validation Session 2 - PRE-A1 scope expanded per Q1 -->

- **File:** `src/store/game-store.ts:290+`
- **Fix:** Add `version: 1` to Zustand persist config. `migrate()` converts existing numeric `executionId` → string (M2 prerequisite). Full migration logic (timestamps etc.) lands in Phase 2 A1.
- **Code:**
  ```ts
  persist(storeCreator, {
    ...
    migrate: (state, version) => {
      if (version === 0 || version === undefined) {
        ...
        state.actionLog?.forEach((e: any) => {
          if (typeof e.executionId === "number")
  ...
  ```
- **Test:** Verify localStorage has `"version": 1` after first write. Verify existing numeric executionIds are converted to strings on rehydration.
...
<!-- Updated: Validation Session 2 - Defensive fix despite existing guard (Q3) -->

- **File:** `src/hooks/use-timer.ts:33-44`
- **Root cause:** Rapid Debate→Judgment: stale callback nulls `timerRef.current` → Stop inert
- **[VALIDATED]** Existing guard at line 25 (`if (timerRef.current) clearInterval(timerRef.current)`) covers basic case. Apply defensive full fix anyway for edge cases.
- **Fix:** Guard `startTimer`: verify existing clearInterval guard, add functional ref update, add cleanup in `useEffect` return. Defensive approach — cover unmount + rapid remount scenarios.
...
- **[RED-TEAM] Correction:** `stopSound(name)` ALREADY EXISTS at `sounds.ts:29-35` (param is `name: SoundName`, not `id`). Do NOT re-add.
- **Fix:**
...
- **Fix:** Add alive-check at start of `executeAction`. Dead source → return `{ blocked: true, reason: 'dead_source' }`. UI shows confirm BottomSheet with generic message. If confirmed, allow.
- **Decision:** Confirm dialog, not hard block (Hunter death shot valid)
- **[VALIDATED] Dialog copy:** Generic — "Nguồn [Tên] đã chết. Vẫn thực hiện [Tên kỹ năng]?" — không cần role-specific context.
...
<!-- Updated: Validation Session 2 - types/game.ts is source of truth (Q4) -->

- **File:** `src/types/game.ts:83-88`
- **[VALIDATED]** `types/game.ts` is authoritative source of truth. `game-store.ts` inline types (lines 33-36) must import from `types/game.ts`, not define separately.
- **Fix:** Add `timestamp: number` to `StatusChangeLog` and `RoleChangeLog` interfaces in `types/game.ts`. Update `game-store.ts` to import these types instead of defining inline.
...
- **Fix:** Extend existing `limited` guard in `executeAction` to also cover `nightly`:
  ```ts
  if (
    (ability.type === "limited" || ability.type === "nightly") &&
...
<!-- Updated: Validation Session 2 - Undo consistent behavior (Q11) -->

- **File:** `src/store/game-store-actions.ts:167`
- **Fix:** Create 1 ActionLog entry with `targets: []` for traceability + undoability. Usage incremented normally.
- **[VALIDATED] Undo:** Normal undo behavior — xóa log entry + decrement abilityUsage. Consistent regardless of target count.
- **Decision:** Create log entry, undo works same as non-empty targets
...
- [x] PRE-A1: Add `version: 1` + no-op migrate to Zustand persist (BEFORE any data schema changes)

### Critical

- [x] C1: Fix deleteRole to clear orphaned player roleIds
- [x] C2: Memoize SkillSheet wizard state, prevent reset on player changes
- [x] C3: Guard timer interval creation, prevent leak on rapid start

### High

- [x] H1: Fix multi-target undo to remove entire execution group + confirm
- [x] H2: Verify existing stopSound + add stopAllSounds + wire visibilitychange (red-team)
- [x] H3: Polyfill requestIdleCallback for Safari
- [x] H4: Fix 5 failing test expectations (timestamp + aria-label)
- [x] H5: Add dead source confirm dialog in executeAction
- [x] H6: Guard focus trap for 0-element states + null-check rAF
- [x] H7: Stabilize onClose ref in bottom-sheet
- [x] H8: Lazy-mount all 6 sheets in game-screen (also fixes P8)

### Medium

- [x] M1: Auto-increment custom role order
- [x] M2: Replace \_counter at BOTH sites (uid + executeAction:165) with `crypto.randomUUID?.() ?? uid()` fallback; change ActionLog.executionId type number→string
- [x] M3+M6: Consolidated cleanupPlayerReferences utility covering all player-id-referencing state (red-team)
- [x] M4: Memoize per-player action arrays in selectors
- [x] M6: Consolidated with M3 (see above)
- [x] M7: Translate SkillSheet step 3 ability name
- [x] M8: Add timestamp to StatusChangeLog/RoleChangeLog types
- [x] M9: Remove hardcoded "Kỹ năng" from addAbility
- [x] M10: Extend limited-guard in executeAction to cover nightly type (use existing abilityUsage, no new state) + warning confirm
- [x] M11: Create ActionLog entry for empty-target executions
- [x] M12: Add Escape key handler to timer overlay

### i18n

- [x] I1: Fix history-sheet hardcoded English (3 strings)
- [x] I2: Fix create-role-sheet hardcoded Vietnamese (5 strings)
- [x] I3: Fix role-library-sheet hardcoded English (2 strings)
- [x] I4: Fix main.tsx PWA prompt with i18n t()
- [x] I5: Fix game-store addAbility hardcoded name
- [x] Add all new i18n keys to vi.json + en.json

### UX Bugs

- [x] U4: Add confirm dialog before role deletion
- [x] U7: Allow targetCount 0 reset in SelectorModal

### Validation

- [x] Run `npm test` — all tests pass (including fixed H4)
- [x] Run `npm run build` — no compile errors
- [x] Manual test: Safari loads without errors (H3)
- [x] Manual test: Timer always stoppable after rapid taps (C3)
- [x] Manual test: Wizard doesn't reset mid-use (C2)
- [x] Manual test: Night sound stops on day/reset (H2)
- [x] Manual test: Role deletion cleans players (C1)
